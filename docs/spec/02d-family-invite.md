...
| D6 | 邀请短码 8 位（字符集 `ABCDEFGHJKLMNPQRSTUVWXYZ23456789`），一用户一码、永久有效、可重复解析；每次解析更新 `used_at` | 稳定分享码，简单优先 | 无 |
| D7 | 家庭邀请链接直接使用 `linkId = familyId`；任何成员均可生成；**长期有效、无撤销**（已接受取舍，ADR-0010） | 无需额外邀请令牌表；familyId 为 UUID v7 | ADR-0010 |
| D8 | 家庭 / 邀请事务使用 `WithTxDeferrable`；缓存失效与物理文件删除在事务提交之后 | 非幂等副作用在事务提交后执行 | 无 |
...
时序（源：`family/service.go:87` `GetFamily`、`family/handler.go:57` `GetFamily`）：
...
时序（`family/service.go:135` `CreateFamily`）：
...
时序（`family/handler.go:182` `CreateInviteLink`）：
...
时序（`family/handler.go:219` `JoinByInviteLink` → `family/service.go:197` `JoinFamily`）：
...
5. `WithTxDeferrable`：重算 current/personal；若 `current==target` 返回 `ErrAlreadyInTargetFamily`；判断是否源家庭 owner；`CountFamilyMembers(target)`；owner 时再取 source count，`target+source > 6` → `ErrFamilyFull`；非 owner 时 `target >= 6` → `ErrFamilyFull`；执行 `joinFamilyTx`。
6. `joinFamilyTx`：
...
时序（`family/handler.go:258` `grantJoinReward`，在 `JoinByInviteLink` 成功后同步调用）：

1. 若当前用户已有 `user_invites` 记录 → 直接返回（不奖励）。
2. 读用户；`time.Since(created_at) > 5 分钟` → 返回。
3. `GetFamilyOwner(目标家庭)`；失败 / 空 / owner == 自己 → 返回。
4. `WithTxDeferrable`：再次检查 user_invites；确认 owner 存在；插入 `user_invites`；被邀请人 +3 天 VIP → `MarkInviteeRewarded`；`LockInviterReward`；`CountInviterMonthlyRewardDays`；`< 14` 时 `MarkInviterRewarded` + 邀请人 +7 天。
5. 任何失败仅 `slog.WarnContext`，不影响主流程 200。

**AC**

- 注册 5 分钟内首次通过链接加入 → 被邀请人 +3 天、邀请人 +7 天（当月上限 14 天）。
- 已有邀请关系 / 注册超过 5 分钟 / 不存在 owner → 不发放，接口仍 200。
...
时序（`family/service.go:413` `LeaveFamily`）：
...
- `personal_family_id` 为空时跳过重入，仅更新 current（避免空 family_id 外键失败）。

### F-7 移除成员（仅 owner）

**故事**：家庭 owner 把某个成员移出家庭。

时序（`family/service.go:457` `RemoveMember`）：
...
3. `ResolveInviterFromCode`：`strings.ToUpper(TrimSpace(code))`；SQL `UPDATE user_invite_codes SET used_at=now() WHERE short_code=$1 AND (expires_at IS NULL OR expires_at > now()) RETURNING user_id`。
4. `ErrNoRows` → 返回空 → 404 `invite code not found`；SQL 错误 → 500。
5. 成功 → 200 `{userId}`。

**AC**

- 8 位合法且存在 → 200 `{userId: 邀请人id}`，同时该码 `used_at` 更新为当前时间（GET 带写副作用）。
- 8 位合法但不存在 / 已过期 → 404。
- 长度或字符集不合法（含小写）→ 400。
...
4. `ensureShortCode`：`GetUserInviteCode` 命中即复用；否则 `util.NewShortCode(8)` 插入；唯一冲突则回查已有码。
...
**现状（代码事实）**：产品不需要家庭级配置；后端**没有**家庭级配置端点，也没有任何 `familyConfig` 数据模型。前端 `pages/Set/Set.ts` 声明的 `familyConfig` 实际来自 `GET /auto-record/config`（自动记录开关，属 PP-02C，用户级而非家庭级），且加载后**未被渲染 / 使用**（仅存入 `data.familyConfig`）。
...
| 字段 | 类型 | 约束 / 说明 |
|---|---|---|
| `family_id` | text | PK 组成；无显式 FK（迁移中未见） |
...
- `user_invite_codes`：一人一码、`short_code` 唯一、`expires_at` 恒 NULL、`used_at` 记录最近解析时间。
- `user_invites`：`user_id` 唯一（一人一个邀请人），`reward_inviter_at` / `reward_invitee_at` 控制奖励幂等。
...
| 二维码缓存 TTL | 6 天 | `invite/qrcode.go:35` |
| 链接加入奖励窗口 | 注册后 5 分钟 | `family/handler.go:270` |
...
| 加入 / 创建：家庭已满 | `ErrFamilyFull` | 409 | 4090 | `FAMILY_FULL` |
| 退出 / 创建：非普通家庭 | `ErrNotInNormalFamily` | 400 | 4000 | — |
...
- 加入流程在加锁前读取源家庭快照，锁后事务内重新计算 current/personal；源家庭状态不在锁后重读，并发一致性由 `family_members.user_id` 唯一约束与 RepeatableRead 事务保证。
...
- 旧物理文件删除在 DB 提交后执行，跳过与新文件同路径的文件（`uploads/YYYY/MM/{shortCode}.{ext}` 确定性命名）。
